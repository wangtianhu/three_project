...
let SpinControls = function (object, trackBallRadius, camera, domElement) {
    var _this = this
    ...
    this.applyVelocity = (function () {
        ...
        return function applyVelocity() {
            ...
            if (deltaAngle && deltaTime) {
                ...
                if (
                    8 * (1 - _lastQuaternion.dot(_this.object.quaternion)) >
                    _EPS
                ) {
    ...
    this.resetInputAfterCameraMovement = function () {
        if (_isPointerDown) {
            ...
            _this.camera.matrixWorldInverse
                .copy(_this.camera.matrixWorld)
                .invert()

            _pointOnSphere.copy(
                getPointerInSphere(
                    getPointerInNdc(_pointerScreen.x, _pointerScreen.y)
                )
            )
        }
    }
    ...
    var getPointerInSphere = (function () {
        ...
        return function getPointerInSphere(ndc) {
            ...
            if (
                _this._pointerMapping === _this.POINTER_SPHERE_MAPPING.RAYCAST
            ) {
            ...
            // Pointer mapping code below derived from Yasuhiro Fujii's https://mimosa-pudica.net/3d-rotation/
                ...
                _this._pointerMapping === _this.POINTER_SPHERE_MAPPING.HOLROYD
            ) {
    ...
    this.onPointerMove = (function () {
        ...
        return function onPointerMove(pointerScreenX, pointerScreenY, time) {
            ...
            if (
                objToPointer.lengthSq() < 1 ||
                !this.relativelySpinOffTrackball
            ) {
                ...
                if (_wasLastPointerEventOnSphere) {
                    ...
                    if (deltaTime > 0) {
                            ...
                            ndcToBall =
                                ...
                                Math.atan(
                                    _this.trackballRadius /
                                        objectToCamera.length()
                                ) // Ball field of view angle size
                        } else {
                            //Assume orthographic
                        ...
                        objToPointer.normalize()
                        var deltaRadius =
                            (deltaMouse.dot(objToPointer) * ndcToBall) /
                            deltaTime
                        ...
                        var angle =
                            lastPointOnSphere.angleTo(_pointOnSphere) /
                            deltaTime
